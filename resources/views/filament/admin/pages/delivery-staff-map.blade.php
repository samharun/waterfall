        ...
        <div class="wf-map-stats">
            @foreach ([
                ...
                <div class="wf-map-stat">
                    <span>{{ $label }}</span>
                    <strong data-map-stat="{{ $key }}">{{ $mapData['stats'][$key] }}</strong>
                </div>
            @endforeach
        </div>
...
<style>
    .wf-map-shell { display: flex; flex-direction: column; gap: 16px; }
    .wf-map-stats { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 12px; }
    .wf-map-stat, .wf-map-panel, .wf-map-list-panel { background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; box-shadow: 0 1px 3px rgba(15, 23, 42, .06); }
    .wf-map-stat { padding: 14px 16px; }
    .wf-map-stat span { display: block; color: #64748b; font-size: 11px; font-weight: 700; letter-spacing: .05em; text-transform: uppercase; }
    .wf-map-stat strong { display: block; color: #0f172a; font-size: 26px; font-weight: 800; line-height: 1.1; margin-top: 6px; }
    .wf-map-layout { align-items: stretch; display: grid; gap: 16px; grid-template-columns: minmax(0, 1fr) 340px; }
    .wf-map-panel, .wf-map-list-panel { overflow: hidden; }
    .wf-map-header, .wf-map-list-header { align-items: center; background: #f8fafc; border-bottom: 1px solid #f1f5f9; display: flex; gap: 12px; justify-content: space-between; padding: 14px 16px; }
    .wf-map-header h2, .wf-map-list-header h3 { color: #0f172a; font-size: 14px; font-weight: 800; margin: 0; }
    .wf-map-header p { color: #64748b; font-size: 12px; margin: 3px 0 0; }
    .wf-map-header button { background: #0077B6; border: 0; border-radius: 8px; color: #fff; cursor: pointer; font-size: 13px; font-weight: 700; padding: 8px 14px; }
    .wf-map-canvas { height: min(68vh, 680px); min-height: 460px; width: 100%; }
    .wf-map-error { background: #fef2f2; border-top: 1px solid #fecaca; color: #991b1b; font-size: 13px; padding: 12px 16px; }
    .wf-map-list-header span { background: #e0f2fe; border-radius: 9999px; color: #0369a1; font-size: 12px; font-weight: 800; padding: 3px 10px; }
    .wf-map-list { max-height: min(68vh, 680px); min-height: 460px; overflow-y: auto; }
    .wf-map-person { border-bottom: 1px solid #f1f5f9; cursor: pointer; padding: 13px 16px; }
    .wf-map-person:hover { background: #f0f9ff; }
    .wf-map-person strong { color: #0f172a; display: block; font-size: 13px; font-weight: 800; }
    .wf-map-person small, .wf-map-person span { color: #64748b; display: block; font-size: 12px; margin-top: 4px; }
    .wf-map-status { align-items: center; display: inline-flex; gap: 6px; }
    .wf-map-status::before, .wf-map-marker-dot { border-radius: 9999px; content: ""; display: inline-block; height: 9px; width: 9px; }
    .wf-map-status.online::before, .wf-map-marker-dot.online { background: #16a34a; }
    .wf-map-status.stale::before, .wf-map-marker-dot.stale { background: #f59e0b; }
    .wf-map-status.offline::before, .wf-map-marker-dot.offline { background: #dc2626; }
    .wf-map-marker { align-items: center; background: #fff; border: 2px solid #0077B6; border-radius: 9999px; box-shadow: 0 8px 18px rgba(15, 23, 42, .22); display: flex; height: 28px; justify-content: center; width: 28px; }
    @media (max-width: 1100px) {
        .wf-map-stats { grid-template-columns: repeat(2, minmax(0, 1fr)); }
        .wf-map-layout { grid-template-columns: 1fr; }
        .wf-map-list, .wf-map-canvas { min-height: 360px; }
    }
</style>
@endpush
...
<script>
    (() => {
        ...
        const statusLabel = (status) => status === 'online' ? 'Online' : (status === 'stale' ? 'Stale' : 'Offline');
        ...
        const renderList = (markers) => {
            ...
            list.innerHTML = markers.map((marker) => {
                const delivery = marker.delivery;
                const zone = delivery?.zone || 'No active zone';
                const deliveryNo = delivery?.delivery_no ? ' - ' + escapeHtml(delivery.delivery_no) : '';
                ...
                return `
                    <button type="button" class="wf-map-person" data-staff-id="${escapeHtml(marker.staff_id)}" style="background:transparent;border:0;text-align:left;width:100%">
                        ...
                        <span>${escapeHtml(zone)}${deliveryNo}</span>
                    </button>
                `;
            }).join('');
        ...
        const render = (data) => {
            ...
            if (refreshed) refreshed.textContent = data.refreshed_at ? new Date(data.refreshed_at).toLocaleString() : 'Unknown';
        };
        ...
        const refresh = async () => {
            ...
            try {
                ...
                if (! response.ok) throw new Error('Unable to load staff locations.');
